...
## 网络包在机器上的接收过程
在机器中都是通过网卡接收和发送数据包的，当网卡收到网络包后，通过DMA技术，写入网络包到指定内存地址，内存地址是一个环形缓存，接着告诉操作系统网络包到达

最简单的就是通过中断。为了解决不断中断的文件，linux采用NAPI机制，混合中断和轮询的方式接收网络包，核心是**不采用中断的方式读取数据，而是首先采用中断唤醒数据接收的服务程序，然后通过 poll的方式轮询数据。**